...
rem     Date:    15/09/2015
rem     Version: 1.5a
rem     Remove certain steps to create faster optimize for busy systems.
...
rem mysqldump -vvv -P%PORT% %USERPASS% %DB% > %BACKUPDIR%\%DB%.sql

rem 7za a "%BACKUPDIR%\%DB%_%DATEMONTH%_%FTIME%.zip" "%BACKUPDIR%\%DB%*.sql"
rem del %BACKUPDIR%\%DB%.sql /q
...
rem echo Check Tables
rem mysqlcheck  -P%PORT% %USERPASS% %DB% -vvv
echo Flush Tables
mysqladmin -vvv -P%PORT% -uqrepl flush-tables
echo Show Status
